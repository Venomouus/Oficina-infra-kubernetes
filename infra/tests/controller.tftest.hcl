...
run "controller_has_scoped_target_registration_and_irsa" {
  ...
  assert {
    condition = (
      jsondecode(aws_iam_role.load_balancer_controller.assume_role_policy).Statement[0].Action == "sts:AssumeRoleWithWebIdentity" &&
      jsondecode(aws_iam_role.load_balancer_controller.assume_role_policy).Statement[0].Condition.StringEquals["${local.oidc_host}:sub"] == "system:serviceaccount:kube-system:aws-load-balancer-controller" &&
      jsondecode(aws_iam_role.load_balancer_controller.assume_role_policy).Statement[0].Condition.StringEquals["${local.oidc_host}:aud"] == "sts.amazonaws.com" &&
      jsondecode(aws_iam_role.load_balancer_controller.assume_role_policy).Statement[0].Principal.Federated == aws_iam_openid_connect_provider.cluster.arn
    )
    error_message = "IRSA deve aceitar somente o service account do controlador neste cluster."
  }
  assert {
    condition = (
      toset(jsondecode(aws_iam_role_policy.target_registration.policy).Statement[1].Action) ==
      toset(["elasticloadbalancing:RegisterTargets", "elasticloadbalancing:DeregisterTargets"]) &&
      toset(jsondecode(aws_iam_role_policy.target_registration.policy).Statement[1].Resource) ==
      ...
      alltrue([for action in jsondecode(aws_iam_role_policy.target_registration.policy).Statement[0].Action : strcontains(action, ":Describe")])
    )
  ...
  assert {
    condition = (
      yamldecode(output.load_balancer_controller_values).serviceAccount.annotations["eks.amazonaws.com/role-arn"] == aws_iam_role.load_balancer_controller.arn &&
